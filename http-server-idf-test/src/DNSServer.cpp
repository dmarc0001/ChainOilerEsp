...
bool DNSServer::start( const uint16_t &port, const std::string &domainName, const std::string &resolvedIP )
{
  _port = port;
  _buffer = NULL;
  ...
  return true;  //_udp.begin(_port) == 1;
}
...
void DNSServer::downcaseAndRemoveWwwPrefix( std::string &domainName )
{
  ...
  // kleinbuchstaben
  //
  std::transform( domainName.begin(), domainName.end(), domainName.begin(), []( unsigned char c ) { return std::tolower( c ); } );
  //
  // www raus filtern
  //
  domainName = std::regex_replace( domainName, std::regex( "www." ), "" );
  /*
  std::string from = "www.";
  std::string to = "";
  size_t start_pos = domainName.find( from );

  if ( start_pos == std::string::npos )
    return;
  domainName.replace( start_pos, from.length(), to );
  return;
  */
}
...
void DNSServer::processNextRequest()
{
  ...
  if ( _currentPacketSize )
  {
    ...
    if ( requestIncludesOnlyOneQuestion() )
    {
      ...
      memcpy( &_dnsQuestion->QType, ( void * ) &_buffer[ DNS_HEADER_SIZE + _dnsQuestion->QNameLength ],
              sizeof( _dnsQuestion->QType ) );
      memcpy( &_dnsQuestion->QClass,
              ( void * ) &_buffer[ DNS_HEADER_SIZE + _dnsQuestion->QNameLength + sizeof( _dnsQuestion->QType ) ],
...
{
  return ntohs( _dnsHeader->QDCount ) == 1 && _dnsHeader->ANCount == 0 && _dnsHeader->NSCount == 0 && _dnsHeader->ARCount == 0;
}
