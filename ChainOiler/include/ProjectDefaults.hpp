...
namespace Prefs
{
  ...
  //
  constexpr const int32_t CURRENT_PREFS_VERSION = 3;       //! aktuelle Version der Preferenzen
  constexpr int32_t INVALID_VERSION = 999;                 //! Kennzeichen für ungültige Version
  constexpr const char *PREFS_PARTITION{ "oilprefs" };     //! partitionsbezeichner
  constexpr const char *PREFS_PARTITION_LABEL{ "prefs" };  //! partitionslabel
  //
  // Konfigurierbare Parameter und deren keys
  //
  constexpr const char *PREFS_VERSION_STR{ "pref_ver" };    //! Name fuer Version
  ...
  constexpr const char *AP_CHANNEL_STR{ "ap_channel" };  //! Kanal für WiFi

  constexpr int32_t DEFAULT_AP_CHANNEL = 2;                               //! Default WiFi Kanal
  constexpr const char *AP_MAX_CONNECTIONS_STR{ "ap_max_conns" };         //! name für max connections
  constexpr int32_t DEFAULT_AP_MAX_CONNECTIONS = 4;                       //! Vorgabewert für maximale Verbindungen
