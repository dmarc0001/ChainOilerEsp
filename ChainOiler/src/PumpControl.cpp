...
namespace esp32s2
{
  ...
  PumpControl PumpControl::inst;
  // esp_timer_handle_t PumpControl::timerHandle{nullptr}; //! timer handle
  // volatile bool PumpControl::pumpIsOn{false};

  PumpControl *PumpControl::getInstance()
  {
    {
      // static PumpControl inst;  // Guaranteed to be destroyed.
      return &inst;
    }
  }
   ...
   *
   */
  PumpControl::PumpControl() : timerHandle( nullptr ), pumpIsOn( false )
  {
  ...
  {
    //
    // timer für Pumpe starten
    //
    const esp_timer_create_args_t appTimerArgs = {
        .callback = &( PumpControl::timerCallback ), .arg = nullptr, .dispatch_method = ESP_TIMER_TASK, .name = "led_timer" };
    //
    // timer erzeugen
    //
    ...
    //
    ESP_ERROR_CHECK( esp_timer_start_periodic( timerHandle, 20000 ) );
    //
  }
