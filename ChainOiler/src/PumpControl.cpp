...
namespace esp32s2
{
  ...
  void PumpControl::init()
  {
    ...
    // pulldown, der Transistor am Ausgang ist sher empfindlich
    // TODO: testen ob das so muss
    //
  ...
  void PumpControl::start()
  {
    ...
    //
    const esp_timer_create_args_t appTimerArgs = { .callback = &( PumpControl::timerCallback ),
                                                   .arg = nullptr,
  ...
  void PumpControl::timerCallback( void * )
  {
    ...
    else
    {
      ...
      // beachte den zeitkritischen zugriff auf die Resource (daher ein Mutex...)
      //
      if ( Prefs::Preferences::pumpCycles > 0 )
      {
        ...
        portENTER_CRITICAL( &Prefs::Preferences::oilCycleMutex );
        PumpControl::pumpIsOn = true;
        --Prefs::Preferences::pumpCycles;
        portEXIT_CRITICAL( &Prefs::Preferences::oilCycleMutex );
