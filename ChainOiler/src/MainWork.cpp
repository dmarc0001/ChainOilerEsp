...
namespace ChOiler
{
  ...
  void MainWorker::run()
  {
    ...
    uint64_t computeSpeedTime = esp_timer_get_time() + 1500000ULL;
    uint64_t computeOilerChcekTime = esp_timer_get_time() + 3500000ULL;
    //
    ...
    computeSpeedTime = esp_timer_get_time() + 2000000ULL;
    while ( true )
    {
      ...
      if ( Preferences::getAppMode() != opMode::APMODE )
      {
        ...
        if ( esp_timer_get_time() > computeOilerChcekTime )
        {
          computeOilerChcekTime = esp_timer_get_time() + 3200000ULL;
